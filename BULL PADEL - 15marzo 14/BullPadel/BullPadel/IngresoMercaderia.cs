    ...
    public partial class IngresoMercaderia : Form
    {
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            if(caja != "error")
            {
                ...
                {
                    cmdtext = "insert into gastos(caja,detalle,importe,fecha) values('" + caja + "','" + textBox1.Text + "','" + textBox6.Text.Replace(",", ".") + "','" + fecha + "')";
                    oacceso.ActualizarBD(cmdtext);
                }
                cmdtext = "insert into ingreso(proveedor, nrocomp, fecha, importe, caja) values('" + textBox1.Text + "','" + textBox2.Text + "','" + fecha + "','" + textBox6.Text.Replace(",",".") + "','" + caja + "')";
                oacceso.ActualizarBD(cmdtext);
        ...
        private void textBox5_KeyPress(object sender, KeyPressEventArgs e)
        {
            try
            {
                if (e.KeyChar == 13 && textBox5.Text != "")
                {
                    string cmdtext;
                    if (Convert.ToInt64(textBox5.Text) < 1000000)
                    {
                        cmdtext = "select idarticulos, descripcion from articulos where idarticulos = '" + textBox5.Text + "'";
                    }
                    else
                    {
                        cmdtext = "select idarticulos, descripcion from articulos where codbarra = '" + textBox5.Text + "'";
                    }
                    DataTable dt = new DataTable();
                    dt = oacceso.leerDatos(cmdtext);
        ...
        private void textBox3_KeyPress(object sender, KeyPressEventArgs e)
        {
            if (e.KeyChar == 13)
            {
                if (Convert.ToInt32(textBox3.Text) > 0 || Convert.ToInt32(textBox3.Text) < 0)
                {
                    button3.Focus();
                }
                else
