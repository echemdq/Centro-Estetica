...
namespace BullPadel
{
    public partial class ConsultaVentas : Form
    {
        ...
        private void button1_Click_1(object sender, EventArgs e)
        {
            ...
            else
            {
                ...
                if (comboBox1.SelectedItem == null)
                {
                    ...
                    string cmdtext = "select case when sum(m.total) <> 0 then sum(m.total) else 0 end as total from ventas v inner join bullpadelbd.movifinal m on v.idventas = m.idventas where m.nombre is null and v.fecha between '" + desde + "' and '" + hasta + "' group by v.nrocomp";
                    ...
                    cmdtext = "select v.total from bullpadelbd.ventas v inner join bullpadelbd.movifinal m on v.idventas = m.idventas where m.nombre is not null and v.fecha between '" + desde + "' and '" + hasta + "' group by v.nrocomp";
                    ...
                    foreach (DataRow dr in dt.Rows)
                    {
                        ventas1 = ventas1 + Convert.ToDecimal(dr["total"]);

                    }
                    txt_cancha.Text = ventas1.ToString();
                    ...
                    decimal total = 0;
                    total = ventas + ventas1 - sueldo - gasto;
                ...
                else
                {
                    if (Convert.ToString(comboBox1.SelectedItem) == "1")
                    {
                        ...
                        DateTime h = Convert.ToDateTime(maskedTextBox2.Text);
                        string hasta = h.ToString("yyyy-MM-dd HH:mm:ss");
                        string cmdtext = "select case when sum(m.total) <> 0 then sum(m.total) else 0 end as total from ventas v inner join bullpadelbd.movifinal m on v.idventas = m.idventas where m.nombre is null and v.fecha between '" + desde + "' and '" + hasta + "'  and idcajas = '1' group by v.nrocomp";
                        DataTable dt = new DataTable();
                        dt = oacceso.leerDatos(cmdtext);
                        decimal ventas = 0;
                        foreach (DataRow dr in dt.Rows)
                        ...
                        cmdtext = "select v.total from bullpadelbd.ventas v inner join bullpadelbd.movifinal m on v.idventas = m.idventas where m.nombre is not null and v.fecha between '" + desde + "' and '" + hasta + "' and idcajas = '1' group by v.nrocomp";
                        // string cmdtext = "select case when sum(total) <> 0 then sum(total) else 0 end as total from ventas where fecha between '" + desde + "' and '" + hasta + "'";
                        decimal ventas1 = 0;
                        dt = oacceso.leerDatos(cmdtext);
                        foreach (DataRow dr in dt.Rows)
                    ...
                    else if (Convert.ToString(comboBox1.SelectedItem) == "2")
                    {
                        DateTime d = Convert.ToDateTime(maskedTextBox1.Text);
                        string desde = d.ToString("yyyy-MM-dd HH:mm:ss");
                        DateTime h = Convert.ToDateTime(maskedTextBox2.Text);
                        string hasta = h.ToString("yyyy-MM-dd HH:mm:ss");
                        string cmdtext = "select case when sum(m.total) <> 0 then sum(m.total) else 0 end as total from ventas v inner join bullpadelbd.movifinal m on v.idventas = m.idventas where m.nombre is null and v.fecha between '" + desde + "' and '" + hasta + "'  and idcajas = '2' group by v.nrocomp";
                        DataTable dt = new DataTable();
                        dt = oacceso.leerDatos(cmdtext);
                        decimal ventas = 0;
                        foreach (DataRow dr in dt.Rows)
                        ...
                        cmdtext = "select v.total from bullpadelbd.ventas v inner join bullpadelbd.movifinal m on v.idventas = m.idventas where m.nombre is not null and v.fecha between '" + desde + "' and '" + hasta + "' and idcajas = '2' group by v.nrocomp";
                        // string cmdtext = "select case when sum(total) <> 0 then sum(total) else 0 end as total from ventas where fecha between '" + desde + "' and '" + hasta + "'";
                        decimal ventas1 = 0;
                        dt = oacceso.leerDatos(cmdtext);
                        foreach (DataRow dr in dt.Rows)
        ...
        private void button2_Click(object sender, EventArgs e)
        {
            ...
            document.Add(new Paragraph(chunk));
            chunk = new Chunk("CANCHAS:  " + txt_cancha.Text, FontFactory.GetFont("VERDANA", 20, iTextSharp.text.Font.BOLD, iTextSharp.text.BaseColor.BLACK));
            document.Add(new Paragraph(chunk));
