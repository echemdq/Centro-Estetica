...
namespace BullPadel
{
    public partial class adicion1 : Form
    {
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            if (textBox2.Text != "" && textBox3.Text != "" && textBox4.Text != "" && textBox5.Text != "")
            {
                if (nombre13 != "PADEL")
                {
                    ...
                    foreach (DataRow dr in dt.Rows)
                    {
                        if (Convert.ToString(dr["sum(total)"]) != "")
                        {
                            total = Convert.ToDecimal(dr["sum(total)"]);
                        }
                        label7.Text = "Total: $" + Convert.ToString(dr["sum(total)"]);
                        string vtatotal = Convert.ToString(dr["sum(total)"]);
                        VTATOTAL = Convert.ToDecimal(vtatotal);
        ...
        private void textBox1_KeyUp(object sender, KeyEventArgs e)
        {
            ...
            {
                decimal efectivo = Convert.ToDecimal(textBox1.Text.Replace(".", ","));
                decimal vuelto = efectivo - VTATOTAL;
                label10.Text = "Vuelto: $" + vuelto;
            }
        }

        private void textBox4_DoubleClick(object sender, EventArgs e)
        {
            Cambio_Precio frm = new Cambio_Precio();
            frm.ShowDialog();
            decimal nuevo = 0;
            nuevo = frm.devolver();
            if (nuevo != 0)
            {
                textBox4.Text = nuevo.ToString();
            }
        }
