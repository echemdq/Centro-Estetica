...
namespace Centro_Estetica
{
    public partial class frmInfCajas : Form
    {
        ...
        private void btnTraer_Click(object sender, EventArgs e)
        {
            ...
            dataGridView1.DataSource = oac.leerDatos("select concat(CAST(ptoventa AS CHAR),' - ',CAST(factura AS CHAR)) as Factura, fecha as Fecha, detalle as Detalle, f.total as Total, t.tipo as 'Forma Pago' from facturacion f left join formasdepago fp on f.idfacturacion = fp.idfacturacion left join tipoformaspago t on fp.idtipoformaspago = t.idtipoformaspago where f.fecha between '" + d.ToString("yyyy-MM-dd") + "' and '" + h.ToString("yyyy-MM-dd HH:mm:ss") + "' order by fecha");
            ...
            DataTable dt = oac.leerDatos("select sum(f.total) as total, t.tipo as 'forma' from facturacion f left join formasdepago fp on f.idfacturacion = fp.idfacturacion left join tipoformaspago t on fp.idtipoformaspago = t.idtipoformaspago where f.fecha between '" + d.ToString("yyyy-MM-dd") + "' and '" + h.ToString("yyyy-MM-dd HH:mm:ss") + "' group by t.tipo union select sum(case when tipo = 'INGRESO' then importe else importe * (-1) end) as total, 'EFECTIVO' as 'forma' from movcajas where fecha between '" + d.ToString("yyyy-MM-dd") + "' and '" + h.ToString("yyyy-MM-dd HH:mm:ss") + "'");
